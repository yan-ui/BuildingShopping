...
public class AreaPickerView extends Dialog {
    ...
    protected void onCreate(Bundle savedInstanceState) {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                ...
                if (position != oldProvinceSelected) {
                    if (oldCitySelected != -1) {
                        addressBeans.get(oldProvinceSelected).getChild().get(oldCitySelected).setStatus(false);
                    }
                    if (oldAreaSelected != -1) {
                        addressBeans.get(oldProvinceSelected).getChild().get(oldCitySelected).getChild().get(oldAreaSelected).setStatus(false);
                    }
                    oldCitySelected = -1;
                    oldAreaSelected = -1;
                }
                cityBeans.addAll(addressBeans.get(position).getChild());
                provinceAdapter.notifyDataSetChanged();
                cityAdapter.notifyDataSetChanged();
                areaAdapter.notifyDataSetChanged();
                strings.set(0, addressBeans.get(position).getRegion_name());
                if (strings.size() == 1) {
                    strings.add("请选择");
        ...
        cityAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            ...
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                ...
                if (position != oldCitySelected) {
                    if (oldAreaSelected != -1 && cityBeans.get(position).getChild() != null) {
                        addressBeans.get(oldProvinceSelected).getChild().get(oldCitySelected).getChild().get(oldAreaSelected).setStatus(false);
                    }
                    oldAreaSelected = -1;
                }
                oldCitySelected = citySelected;
                if (cityBeans.get(position).getChild() != null) {
                    areaBeans.addAll(cityBeans.get(position).getChild());
                    cityAdapter.notifyDataSetChanged();
                    areaAdapter.notifyDataSetChanged();
                    strings.set(1, cityBeans.get(position).getRegion_name());
                    if (strings.size() == 2) {
                        strings.add("请选择");
        ...
        areaAdapter.setOnItemClickListener(new BaseQuickAdapter.OnItemClickListener() {
            @Override
            public void onItemClick(BaseQuickAdapter adapter, View view, int position) {
                strings.set(2, areaBeans.get(position).getRegion_name());
                tabLayout.setupWithViewPager(viewPager);
                viewPagerAdapter.notifyDataSetChanged();
    ...
    public void setSelect(int... value) {
        ...
        if (value == null || value.length == 0) {
            ...
            if (isCreate) {
                ...
                if (citySelected != -1)
                    addressBeans.get(provinceSelected).getChild().get(citySelected).setStatus(false);
                cityBeans.clear();
                areaBeans.clear();
                provinceAdapter.notifyDataSetChanged();
                cityAdapter.notifyDataSetChanged();
        ...
        if (value.length == 3) {
            strings.add(addressBeans.get(value[0]).getRegion_name());
            strings.add(addressBeans.get(value[0]).getChild().get(value[1]).getRegion_name());
            strings.add(addressBeans.get(value[0]).getChild().get(value[1]).getChild().get(value[2]).getRegion_name());
            tabLayout.setupWithViewPager(viewPager);
            viewPagerAdapter.notifyDataSetChanged();
            ...
            addressBeans.get(value[0]).getChild().get(value[1]).setStatus(true);
            addressBeans.get(value[0]).getChild().get(value[1]).getChild().get(value[2]).setStatus(true);
            cityBeans.clear();
            cityBeans.addAll(addressBeans.get(value[0]).getChild());
            areaBeans.clear();
            areaBeans.addAll(addressBeans.get(value[0]).getChild().get(value[1]).getChild());
            provinceAdapter.notifyDataSetChanged();
            cityAdapter.notifyDataSetChanged();
            areaAdapter.notifyDataSetChanged();
