 ...
 * @see <pre>
 *       基础使用：
 *       extends BaseRecyclerMVPActivity 并在子类initView()中调用 initRecyclerView(...) , onRefresh(...)
 *       列表数据加载及显示过程：
 ...
 *   </pre>
 */
public abstract class BaseRecyclerMVPActivity<P extends BaseContract.BasePresenter, T, VH extends BaseViewHolder, A extends BaseQuickAdapter<T, VH>>
        extends BaseMVPActivity<P> implements BaseQuickAdapter.OnItemClickListener, BaseQuickAdapter.OnItemLongClickListener, BaseQuickAdapter.OnItemChildClickListener {

    private static final String TAG = "BaseRecyclerMVPActivity";
     ...
     * @return
     */
    private synchronized void handleList(int page, List<T> newList, boolean isCache) {
        if (newList == null) {
            newList = new ArrayList<T>();
        }
        isSucceed = !newList.isEmpty();

        Log.i(TAG, "\n\n<<<<<<<<<<<<<<<<<\n handleList  newList.size = " + newList.size() +
                "; isCache = " + isCache + "; page = " + page + "; isSucceed = " + isSucceed);
